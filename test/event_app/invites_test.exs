defmodule EventApp.InvitesTest do
  ...
  describe "invites" do
    ...
    @valid_attrs %{status: "some status", user_email: "some user_email"}
    @update_attrs %{status: "some updated status", user_email: "some updated user_email"}
    @invalid_attrs %{status: nil, user_email: nil}
    ...
    test "create_invite/1 with valid data creates a invite" do
      ...
      assert invite.status == "some status"
    ...
    test "update_invite/2 with valid data updates the invite" do
      ...
      assert invite.status == "some updated status"
